...
def location():
	...
	if catalog:
		...
		session.remove()
		return render_template('display_wine.html', cat = catalog, wine = wine)
	else:
		new = Catalog(location_name = request.form['name'], user_id = login_session['user_id'])
	...
	if request.method == 'POST':
		new = Wine(wine_maker = request.form['maker'], wine_vintage = request.form['vintage'], 
			wine_varietal = request.form['varietal'], 
	         wine_price = request.form['price'], loc_id = locId, wine = location, user_id = login_session['user_id'])
		this_session.add(new)
		this_session.commit()
