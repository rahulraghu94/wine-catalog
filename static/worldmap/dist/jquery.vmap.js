  ...
  jQuery(params.container).delegate(this.canvas.mode === 'svg' ? 'path' : 'shape', 'click', function (regionClickEvent) {
    ...
    if ( !mapClickEvent.isDefaultPrevented()) {
      if (map.isSelected(code)) {
        ...
        var data = {name: mapData.paths[code].name};
        $.ajax({
          type: 'POST',
          url: '/location',
          processData: false,
          ...
          success: function(result){
            ...
            if(result){
              ...
              $('#result').html('Login Successful yayay!</br>'+result+"</br>Redirecting...")
              setTimeout(function()
                { var uri = "/list?name="
                  uri = uri + mapData.paths[code].name
                  window.location.href = uri;}, 4);
